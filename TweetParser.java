...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        if (splittedcsvLine.length <= csvColumn) {
            return null;
        }
        if (splittedcsvLine[csvColumn].equals("")) {
            return null;
        }
        return splittedcsvLine[csvColumn]; // Complete this method.
    }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (li.hasNext()) {
            String rowData = li.next();
            if (extractColumn(rowData, tweetColumn) != null) {
                columnData.add(extractColumn(rowData, tweetColumn));
            }
        }
        return columnData; // Complete this method.
    }
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        List<String> goodWords = new LinkedList<>();
        ...
        return goodWords; // Complete this method.
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        for (String line : tweetSplit(removeURLs(tweet))) {
            sentences.add(parseAndCleanSentence(line));
        }
        return sentences; // Complete this method.
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        for (String sentence : csvDataToTweets(br, tweetColumn)) {
            trainingData.add(parseAndCleanSentence(sentence));
        }

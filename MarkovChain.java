...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        ProbabilityDistribution<String> dist = chain.get(first);
        if (dist == null) {
            dist = new ProbabilityDistribution<>();
            chain.put(first, dist);
        }
        dist.record(second);
    ...
    public void train(Iterator<String> sentence) {
        ...
        while (sentence.hasNext()) {
            ...
            if (word != null) {
                startWords.record(word);
            }
        }

        if (last != null) {
            addBigram(last, END_TOKEN);
        }
    ...
    public boolean hasNext() {
        ...
        if (currentWord.equals(END_TOKEN)) {
            return false;
        }
    ...
    public String next() {
        ...
        ProbabilityDistribution<String> dist = get(currentWord);
        if (dist == null) {
            currentWord = END_TOKEN;
        } else {
            currentWord = dist.pick(ng);
        }
